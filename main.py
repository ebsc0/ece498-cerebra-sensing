...
from config import NUM_OPTODES, SAMPLE_RATE_HZ, PACKET_FORMAT, UI_UPDATE_RATE_HZ, ACTIVE_OPTODES
...
class CerebraApp(App):
    ...
    def build(self):
        ...
        self.buffer = Buffer(num_optodes=NUM_OPTODES)
    ...
    def stop_collection(self):
        ...
        if self.session_id:
            ...
            self.screen.append_log(
                ...
                f"Duration: {elapsed}, Frames: {self.frame_count})\n"
            )
    ...
    def _on_packet(self, packet: bytes):
        ...
        if complete_frame:
            ...
            if preprocessed:
                self.preprocessed_queue.put((complete_frame, preprocessed))
    ...
    def _store_frame(self, frame: CompleteFrame) -> Dict[int, PreprocessedResult]:
        ...
        sample_ids = {}  # {optode_id: sample_id}

        # Insert each optode's raw sample
        for optode_id, packet in frame.packets.items():
            ...
            sample_id = self.db.insert_raw_sample(
                session_id=self.session_id,
        ...
        for optode_id, result in preprocessed.items():
            ...
            self.db.insert_preprocessed_sample(
